...
#[test]
fn webpipe_eval_matrix_export_contract_stubbed_localhost() {
    ...
    // Export judge dataset (joined with qrels).
    ...
    export_cmd.assert()
        .success()
        .stdout(predicate::str::contains("export.jsonl"));

    let raw = std::fs::read_to_string(&export).expect("export exists");
    let line0 = raw.lines().find(|l| !l.trim().is_empty()).expect("row");
    let v: serde_json::Value = serde_json::from_str(line0).expect("json");
    assert_eq!(v["schema_version"].as_u64(), Some(1));
    assert_eq!(v["kind"].as_str(), Some("webpipe_eval_matrix_example"));
    assert!(v["query_id"].is_string());
    assert!(v["case"].is_string());
    assert!(v["judge_text"].is_string());
    assert!(v["hit_expected_url"].is_boolean());
}
