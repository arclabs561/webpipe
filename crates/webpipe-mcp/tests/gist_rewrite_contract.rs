...
#[test]
fn web_extract_rewrites_gist_to_raw() {
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(async {
        let raw = "hello from gist raw\n";
        ...
        let service = ()
            .serve(TokioChildProcess::new(
                tokio::process::Command::new(bin).configure(|cmd| {
                    ...
                    cmd.env(
                        "WEBPIPE_CACHE_DIR",
                        std::env::temp_dir().join("webpipe-gist-cache"),
                    );
                    cmd.env("WEBPIPE_GIST_REWRITE_HOSTS", "127.0.0.1");
                    cmd.env("WEBPIPE_GIST_RAW_HOST", &raw_host);
                }),
            )?)
            .await?;
        ...
        let gist_url = format!("http://{addr}/user/abc");
        let v = call(
            &service,
            "web_extract",
            serde_json::json!({
        ...
        assert!(
            v["final_url"]
                .as_str()
                .unwrap_or("")
                .contains("/user/abc/raw"),
            "expected final_url to be gist raw: {}",
            v["final_url"]
        );
        let empty = vec![];
