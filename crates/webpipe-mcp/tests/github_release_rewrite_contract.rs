...
#[test]
fn web_extract_rewrites_github_release_to_api_json() {
    ...
    rt.block_on(async {
        ...
        let app = Router::new().route(
            "/api/repos/owner/repo/releases/tags/v1.2.3",
            get(move || {
                let body = rel_s.clone();
        ...
        let service = ()
            .serve(TokioChildProcess::new(
                tokio::process::Command::new(bin).configure(|cmd| {
                    ...
                    cmd.env(
                        "WEBPIPE_CACHE_DIR",
                        std::env::temp_dir().join("webpipe-github-release-cache"),
                    );
        ...
        let v = call(
            ...
            "web_extract",
            serde_json::json!({
                "url": "http://github.com/owner/repo/releases/tag/v1.2.3",
                "fetch_backend": "local",
                "timeout_ms": 10_000,
                "max_bytes": 200_000,
                "max_chars": 10_000,
        ...
        assert!(
            v["final_url"]
                .as_str()
                .unwrap_or("")
                .contains("/api/repos/owner/repo/releases/tags/v1.2.3"),
            "expected final_url to be api release: {}",
            v["final_url"]
        );
        let empty = vec![];
    ...
    .expect("github release rewrite contract");
}
